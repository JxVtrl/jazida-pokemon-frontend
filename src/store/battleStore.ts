...
export const useBattleStore = create<BattleStore>((set, get) => ({
  ...
  fetchMyPokemons: async () => {
    try {
      ...
      console.log("👤 Trainer no store:", get().trainer);

      const response = await api.get("/me/pokemons");
      console.log("📦 Pokémons do treinador recebidos:", response.data);
      console.log("📊 Quantidade de pokémons:", response.data?.length || 0);
  ...
  connectSocket: () => {
    ...
    console.log(
      "🔗 Conectando socket para treinador:",
      trainer.id,
      trainer.nome,
    );
    ...
    socket.on(
      ...
      (data: {
        ...
        console.log(
          `🥊 Round ${data.round}: ${data.atacantePrimeiro} atacou primeiro`,
        );
        console.log(`🥊 Dano A: ${data.danoA}, Dano B: ${data.danoB}`);
        console.log(
          `🥊 Vida A: ${data.pokemonA.vida}, Vida B: ${data.pokemonB.vida}`,
        );
    ...
    socket.on(
      "battle-player-left",
      (data: { battleId: string; trainerId: number; message: string }) => {
  ...
  joinBattle: (battleId: string) => {
    ...
    if (!socket.connected) {
      ...
      socket.once("connect", () => {
        console.log(
          "🔗 Socket conectado, agora entrando na batalha:",
          battleId,
        );
