...
export default function BattleHUD() {
  ...
  const [selectedPokemonA, setSelectedPokemonA] = useState<Pokemon | null>(
    null,
  );
  const [selectedPokemonB, setSelectedPokemonB] = useState<Pokemon | null>(
    null,
  );
  ...
  const handleBattle = async () => {
    ...
    try {
      ...
      const response = await api.post(
        `/batalha/${selectedPokemonA.id}/${selectedPokemonB.id}`,
      );
      ...
      const errorMessage =
        (err as { response?: { data?: { error?: string } } })?.response?.data
          ?.error || "Erro ao realizar batalha";
      ...
      <Card>
        ...
        <CardContent>
                ...
                {pokemons.map((pokemon) => (
                  <Card
                    ...
                    className={`cursor-pointer transition-all hover:scale-105 ${
                      selectedPokemonA?.id === pokemon.id ||
                      selectedPokemonB?.id === pokemon.id
          ...
          {(selectedPokemonA || selectedPokemonB) && (
            <div className="mb-6">
              <h3 className="text-lg font-semibold mb-4">
                Pokémons Selecionados:
              </h3>
              <div className="flex flex-col md:flex-row gap-4 justify-center">
                {selectedPokemonA && (
                  <Card
                    className={`${getTypeColor(selectedPokemonA.tipo)} border-2 border-blue-500`}
                  >
                    <CardContent className="p-4 text-center">
                      <div className="text-3xl mb-2">
                        {getPokemonImage(selectedPokemonA.tipo)}
                      </div>
                      <h4 className="font-bold capitalize">
                        {selectedPokemonA.tipo}
                      </h4>
                ...
                {selectedPokemonB && (
                  <Card
                    className={`${getTypeColor(selectedPokemonB.tipo)} border-2 border-red-500`}
                  >
                    <CardContent className="p-4 text-center">
                      <div className="text-3xl mb-2">
                        {getPokemonImage(selectedPokemonB.tipo)}
                      </div>
                      <h4 className="font-bold capitalize">
                        {selectedPokemonB.tipo}
                      </h4>
                  ...
                  <Card className="border-2 border-yellow-400 bg-yellow-50">
                    <CardContent className="p-4 text-center">
                      ...
                      <p className="text-xs text-gray-600">
                        Probabilidade:{" "}
                        {(
                          battleResult.batalha.probabilidadeVencedor * 100
                        ).toFixed(1)}
                        %
                      </p>
                    </CardContent>
                  ...
                  >
                    <CardContent className="p-4 text-center">
                      ...
                      <p className="text-xs text-gray-600">
                        Probabilidade:{" "}
                        {(
                          battleResult.batalha.probabilidadePerdedor * 100
                        ).toFixed(1)}
                        %
                      </p>
                    </CardContent>
