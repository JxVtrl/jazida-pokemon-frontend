...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  const { setTrainer, connectSocket, disconnectSocket, socket } =
    useBattleStore();
  ...
  const login = async (nome: string, senha: string) => {
    try {
      ...
      const { token, treinador } = response.data;
      ...
      return {
        success: false,
        error: errorResponse.response?.data?.error || "Erro no login",
      };
    }
  };
  ...
  const register = async (nome: string, senha: string) => {
    try {
      setLoading(true);
      const response = await api.post("/auth/register", { nome, senha });
      const { token, treinador } = response.data;
      ...
      return {
        success: false,
        error: errorResponse.response?.data?.error || "Erro no registro",
      };
    }
  };
